    ...
    if (cardRef.current && contentRef.current) {
      ...
      tl.to(cardRef.current, {
        ...
        .to(
          ...
          "-=0.2"
        ); // Slight overlap with the card animation
    }
  }, [index]);
    ...
    <Paper
      ...
      style={{
        backgroundColor: hovered
          ...
          : theme.colors.dark[8],
        border: `1px solid ${
          hovered ? theme.colors.blue[5] : theme.colors.dark[7]
        }`,
        transition: "all 0.3s ease",
        cursor: "pointer",
        transform: hovered ? "translateY(-5px)" : "translateY(0)",
        boxShadow: hovered
          ? "0 10px 20px -10px rgba(67, 97, 238, 0.4)"
          : "none",
        perspective: "1000px",
      }}
    >
      <Stack ref={contentRef} gap="xs" align="center">
        <Text fw={600} c={theme.white} ta="center">
          {name}
        </Text>
        <Badge variant="filled" color={hovered ? "blue" : "dark"} radius="sm">
          {count} students
        </Badge>
      </Stack>
    </Paper>
